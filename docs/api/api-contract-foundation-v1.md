...
Các mã lỗi dùng trong foundation: `AUTH_INVALID_CREDENTIALS`, `AUTH_TOKEN_EXPIRED`, `FORBIDDEN_PERMISSION`, `FORBIDDEN_SCOPE`, `RESOURCE_NOT_FOUND`, `VALIDATION_ERROR`, `CONFLICT`, `USER_EMAIL_EXISTS`, `TEAM_NAME_EXISTS`.
...
|---|---|---|---|
| `GET` | `/users` | `identity.user.manage` | Danh sách user trong organization; Manager chỉ thấy team scope. |
| `POST` | `/users` | `identity.user.manage` | Tạo user mới trong organization. |
| `GET` | `/users/{userId}` | `identity.user.manage` hoặc self | Chi tiết user trong scope. |
| `PATCH` | `/users/{userId}` | `identity.user.manage` | Sửa full name, status và metadata an toàn. |
| `PUT` | `/users/{userId}/roles` | `identity.role.manage` | Thay role của user. |
...
Backend chuẩn hóa email, kiểm tra unique trong organization, băm password và ghi audit event `USER_CREATED`. `initialPassword` không xuất hiện trong response hoặc audit log.
...
Response trả user summary mới. Backend từ chối nếu role không thuộc organization hoặc actor thay đổi quyền ngoài scope; ghi audit event `USER_ROLES_CHANGED`.

### Role và Permission

| Method | Path | Permission | Mục đích |
|---|---|---|---|
| `GET` | `/roles` | `identity.role.manage` | Danh sách role trong organization. |
...
Role mặc định của MVP: `ADMIN`, `MANAGER`, `SALES_REP`, `CUSTOMER_CARE`, `DATA_STAFF`.
...
## 6. Audit và test contract bắt buộc

- Mutation Auth/User/Role/Permission/Organization/Team tạo audit log với actor, action, entity và timestamp.
- Test: login sai credential; token hết hạn; Sale đọc user ngoài scope; Manager sửa team ngoài scope; Admin thay role; tạo user email trùng; cập nhật member team không cùng organization.
